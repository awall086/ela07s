...
# Adapted to ELA07S by Adrian Fernandez (awall86)
...
if (!CONFIG.noUpdateCheck) {
	http.load("https://raw.githubusercontent.com/awall086/ela07s/master/revision.txt").done(func(r) setprop("/systems/acconfig/new-revision", r.response));
}
...
setlistener("/sim/signals/fdm-initialized", func {
	...
	if (getprop("/systems/acconfig/out-of-date") == 1) {
		update_dlg.open();
		print("System: The ELA07S is out of date!");
	} 
	mismatch_chk();
	readSettings();
	if (getprop("/systems/acconfig/out-of-date") != 1 and getprop("/systems/acconfig/options/revision") != current_revision and getprop("/systems/acconfig/mismatch-reason") == "XX" and !CONFIG.noUpdateCheck) {
		updated_dlg.open();
		print("System: The ELA07S is up to date!");
	}
...
var readSettings = func {
	io.read_properties(getprop("/sim/fg-home") ~ "/Export/ela07s-config.xml", "/systems/acconfig/options");
}

var writeSettings = func {
	io.write_properties(getprop("/sim/fg-home") ~ "/Export/ela07s-config.xml", "/systems/acconfig/options");
}
